...
const generateWithGemini = async (body, modelName, apiKey, baseUrl) => {
    ...
    if (isThinking) {
        // Thinking only supported on specific Google models, we assume user picked a compatible one if they enabled this mode
        // Or if they used the default.
    ...
    try {
        ...
        // Fallback Logic for Thinking timeout handled by recursion in main handler if needed, 
        // but here we just throw to let caller decide.
        throw error;
    }
};
...
const generateWithOpenAI = async (body, modelName, apiKey, baseUrl) => {
    ...
    const payload = {
        ...
        // Note: Thinking parameters are not standard across OpenAI compatibles, ignoring 'mode' param
    };
...
app.post('/api/generate', async (req, res) => {
  try {
    ...
    // 1. Use passed modelConfig
    // 2. Fallback to server env for Google if no key provided in config
        ...
        try {
            result = await generateWithGemini(req.body, modelName, apiKey, baseUrl);
             ...
             if (mode === 'THINKING') {
                ...
                // Retry with Thinking disabled and ensure model is flash-lite or flash
